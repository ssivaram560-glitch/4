...
const SITE_URL    = "https://thirteen-ultra.vercel.app/";
const CHROME_ARGS = [
...
async function placeBet(userId, chatId, period, prediction, predType, level, amountOverride) {
    ...
    for (let i = 0; i < maxRetries; i++) {
        try {
            ...
            // === BET RESPONSE-IL IRUNDHU TOKEN-AH EDUTHU UPDATE PANRA CODE ===
            const extractedToken = d.token || d.data?.token || d.accessToken || d.access_token;
            const responseToken = normalizeToken(extractedToken);
            
            if (responseToken && responseToken !== token && responseToken.length >= 20) {
                console.log(`[BET] 🔄 New token received from bet response! Updating... length=${responseToken.length}`);
